...
<!--
```agda
private module _ {a b : ⌞ C ⌟} {w x : ⌞ D ⌟} (f : C.Hom a b) (g : D.Hom w x) where
```
-->

Note that for a bifunctor $F$, with $G$ its opposite bifunctor, $F f g$
and $G f g$ will generally _not_ be definitionally equal, instead

```agda
  _ : Bifunctor._◆_ F f g ≡ Bifunctor._◆_ bop f g
  _ = Bifunctor.lrmap F f g
```
